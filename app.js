(function () {
  ...
  // ── ADD TO CART ─────────────────────────────────────────────────────────
  window.addToCartFromIndex = function() {
    var sk = document.getElementById("servicio").value;
    if (!sk) { showToast("Please select a service first.", "error"); return; }
    var sd = servicesData[sk] || {};
    ...
    if (isRoomBasedService(sk)) {
      ...
      numericAmount = total; finalPrice = formatGYD(total);
      ...
      numericAmount = sides * (sd.ratePerSide || 0); finalPrice = formatGYD(numericAmount);
      serviceName = sd.name + " (" + sides + " side" + (sides > 1 ? "s" : "") + ")";
    } else if (isSqftService(sk)) {
      var l = parseFloat((document.getElementById("lengthFeet") || {}).value) || 0;
      var w = parseFloat((document.getElementById("widthFeet") || {}).value) || 0;
      if (l <= 0 || w <= 0) { showToast("Please enter valid Length and Width in feet.", "error"); return; }
      numericAmount = calculateSqftPrice(sk, l, w); finalPrice = formatGYD(numericAmount);
    } else {
    ...
    // Reset only the service-selection part of the form (keep name/email/etc. intact)
